...
void RNNAlgorithm::run(TSPData &data){
    ...
    for (int startCity = 0; startCity < cities; startCity++){
        ...
        if (currentCost != INT_MAX && currentCost < length) {
            length = currentCost;
            for (int i = 0; i < cities; i++) {
...
void RNNAlgorithm::repetition(TSPData &data, int begin) {
    ...
    for (int i = 1; i < cities; i++) {
        ...
        for(int j=0;j<cities;j++){
            if(!visited[j] && map[currentCity][j]<minDistance && map[currentCity][j]!=-1){
                nearestCity=j;
                minDistance=map[currentCity][j];
            }
        }

        //when there is no path - stop searching
        if (nearestCity == -1) {
            currentCost = INT_MAX;
            return;
        }
